...
app.get('/registry/:project/:artifactType', (req, res) => {
  ...
  if (!projectArtifacts) { return res.status(404).json({ error: "Project not found in NM-Registry" }); }
  const artifact = projectArtifacts[artifactType];
  if (!artifact) { return res.status(404).json({ error: `Artifact ${artifactType} not found for project ${project}` }); }
